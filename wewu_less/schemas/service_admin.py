...
class ServiceAdminSchema(Schema):
    ...
    def validate_only_one_contact_method(self, data, **kwargs):
        if data.get("email", None) is not None and data.get("phone_number", None) is not None:
    ...
    def validate_phone_number(self, phone_number: str | None):
        ...
        if phone_number is not None:
            is_valid = len(phone_number) == 9 and all([c.isdigit() for c in phone_number])

        if not is_valid:
            raise ValidationError("Invalid phone number, it should consist of 9 digits", field_name="phone_number")
